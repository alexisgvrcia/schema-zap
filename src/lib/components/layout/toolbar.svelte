...
>
  <div class="flex items-center gap-0.5 sm:gap-1">
    <Tooltip content="Reset view">
      <Button variant="icon" size="md" onClick={resetCanvas} aria-label="Reset view">
    ...
    <Tooltip content={$showBadgets ? 'Hide badges' : 'Show badges'}>
      <Button
        onClick={toggleBadgets}
        aria-label={$showBadgets ? 'Hide badges' : 'Show badges'}
        class={cn(
